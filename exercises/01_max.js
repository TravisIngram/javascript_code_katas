...
function max(list) {
  var maxSoFar = list[0];                  // or, equivalently, list[0]
  for (var i = 0; i < list.length; i++) {  // or, equivalently, for item in list
    if (list[i] > maxSoFar) {              // if current item is greater than max so far
      maxSoFar = list[i];                  // set max so far to current item
    }
  }

  return maxSoFar;                         // we've gone through entire list, return max so far
}
